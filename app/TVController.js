...
export default class TVController {
  constructor(saeConstants, $scope, $rootScope, $http, $timeout, $mdDialog, uiGridTreeViewConstants, graph, lookup, formGrid, summaryGrid) {
    ...
    tvc.readonly = false;
    tvc.selectedItem = null;
    tvc.searchText = null;
    tvc.querySearch = querySearch;
    tvc.vegetables = loadVegetables();
    tvc.selectedVegetables = [];
    tvc.numberChips = [];
    tvc.numberChips2 = [];
    tvc.numberBuffer = '';
    tvc.autocompleteDemoRequireMatch = true;
    tvc.transformChip = transformChip;
    ...
    function querySearch(query) {
      var results = query ? tvc.vegetables.filter(createFilterFor(query)) : [];
      return results;
    }
